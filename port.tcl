...
#		putquick "NOTICE $nick :Usage: $portcheck_setting(cmd_pub) <host> <port>"
		...
		if {[catch {set sock [socket -async $host $port]} error]} {
			putquick "PRIVMSG $chan :4,9 Port   Connection to $host \($port\) was refused."
		} else {
...
proc portcheck_scan_dcc {hand idx text} {
	...
	if {$port == ""} {
		...
		if {[catch {set sock [socket -async $host $port]} error]} {
			putdcc $idx "[portcheck_dopre]4,9 Port   Connection to $host \($port\) was refused."
		} else {
...
proc portcheck_connected {idx sock host port timerid} {
	...
	if {[set error [fconfigure $sock -error]] != ""} {
		close $sock
		putdcc $idx "[portcheck_dopre]4,9 Port   Connection to $host \($port\) failed  \([string totitle $error]\)"
	} else {
		fileevent $sock writable {}
		fileevent $sock readable [list portcheck_read $idx $sock $host $port]
		putdcc $idx "[portcheck_dopre]4,9 Port   Connection to $host \($port\) accepted ."
	}
}
proc portcheck_timeout {idx sock host port} {
	close $sock
	putdcc $idx "[portcheck_dopre]4,9 Port   Connection to $host \($port\) timed out ."
}
...
proc portcheck_connected_pub {chan sock host port timerid} {
	...
	if {[set error [fconfigure $sock -error]] != ""} {
		...
		putquick "PRIVMSG $chan :4,9 Port   Connection to $host \($port\) accepted ."
	}
}
proc portcheck_timeout_pub {chan sock host port} {
	close $sock
	putquick "PRIVMSG $chan :4,9 Port   Connection to $host \($port\) timed out ."
}
...
putlog "\002PORTCHECK:\002 PortCheck.tcl Version 2.0 by uranus is loaded."